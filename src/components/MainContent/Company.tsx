...
export default function Project(props: {name: string, login: string, logo: string, setComp: (arg0: string[]) => unknown}) {
  ...
  return (
    <div className="group flex flex-col gap-2">
        <Image
          onClick={()=>{props.setComp([props.login, props.name])}}
          draggable={false}
          ...
          alt={props.name}
        />
        <span dir='ltr' className="flex flex-row text-center"><a href={url} target="_blank" className="break-normal w-full transition">{props.name}</a></span>
    </div>
  );
}
